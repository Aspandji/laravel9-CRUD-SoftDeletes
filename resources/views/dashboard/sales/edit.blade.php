...
@section('container')
    ...
    <div class="col-lg-8">
        <form action="/dashboard/sales/{{ $sale->slug }}" method="POST" enctype="multipart/form-data" class="mb-5">
            @method('put')
            @csrf
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
                    placeholder="Nama Customer" required autofocus value="{{ old('name', $sale->name) }}">
                @error('name')
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control rounded-top @error('address') is-invalid @enderror" name="address" id="address"
                    placeholder="Jl. Semoga Berkah" style="height: 100px" required value="{{ old('address') }}">{{ $sale->address }}</textarea>
                @error('address')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
